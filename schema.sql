...
insert into equipment (equipment_name) values ('Motor Engine')
...
create policy "auth read uncertainty_records"  on uncertainty_records  for select using (auth.role() = 'authenticated');
create policy "auth read equipment"            on equipment            for select using (auth.role() = 'authenticated');
...
create policy "svc write uncertainty_records"  on uncertainty_records  for all using (auth.role() = 'service_role');
create policy "svc write equipment"            on equipment            for all using (auth.role() = 'service_role');

-- Authenticated users can write uncertainty records and equipment
create policy "auth write uncertainty_records" on uncertainty_records  for all using (auth.role() = 'authenticated');
