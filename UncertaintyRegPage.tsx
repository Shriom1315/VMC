...
function FactorSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
...
export default function UncertaintyRegPage() {
  ...
  // ── Fetch equipment names for dropdown ──
  const fetchEquipment = async () => {
    ...
    if (data) {
      setEquipmentList(data.map((r: any) => r.equipment_name));
    }
  };
  ...
  const handleSave = async () => {
    if (!masterEquipmentName.trim()) {
      setError("Master Equipment Name is required.");
      return;
    }
    setError(null);
    const payload = {
  ...
  const handleSelect = (r: UncertaintyRecord) => {
    ...
    setAccuracyValue(r.accuracy_value);
    setAccuracyFactor(r.accuracy_factor);
    setUncertaintyCertValue(r.uncertainty_cert_value);
    setUncertaintyCertFactor(r.uncertainty_cert_factor);
    setKFactorValue(r.k_factor_value);
    setKFactorFactor(r.k_factor_factor);
    setListCountIucValue(r.list_count_iuc_value);
    setListCountIucFactor(r.list_count_iuc_factor);
    setResolutionValue(r.resolution_value);
    setResolutionFactor(r.resolution_factor);
    setRepeatabilityValue(r.repeatability_value);
    setRepeatabilityFactor(r.repeatability_factor);
  };
    ...
    >
      ...
      <div className="bg-white rounded-xl border border-border shadow-sm">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <span className="text-sm font-semibold text-text-primary">
            {editingId ? "Edit Uncertainty Record" : "New Form"}
          </span>
          {editingId && (
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          {/* Master Equipment Name — fixed width dropdown */}
          <div className="flex items-center gap-4">
            <label className="text-sm font-medium text-text-secondary w-52 shrink-0">Master Equipment Name</label>
            <select
              value={masterEquipmentName}
              onChange={e => setMasterEquipmentName(e.target.value)}
              className="border border-border rounded-md px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-brand-orange focus:border-brand-orange transition-colors bg-white appearance-none cursor-pointer w-96"
            >
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* Row: label | small value input | "Factor Reqirment:" label | small factor dropdown */}
            {[
              {
                label:       "Accuracy / Max. Error (μm)",
                value:       accuracyValue,
                setValue:    setAccuracyValue,
                factor:      accuracyFactor,
                setFactor:   setAccuracyFactor,
              },
              {
                label:       "Uncertainty from cert (μm)",
                value:       uncertaintyCertValue,
                setValue:    setUncertaintyCertValue,
                factor:      uncertaintyCertFactor,
                setFactor:   setUncertaintyCertFactor,
              },
              {
                label:       "k Factor",
                value:       kFactorValue,
                setValue:    setKFactorValue,
                factor:      kFactorFactor,
                setFactor:   setKFactorFactor,
              },
              {
                label:       "List Count OF IUC",
                value:       listCountIucValue,
                setValue:    setListCountIucValue,
                factor:      listCountIucFactor,
                setFactor:   setListCountIucFactor,
              },
              {
                label:       "Resolution",
                ...
                setFactor:   setResolutionFactor,
              },
              {
                label:       "Repeatability",
                value:       repeatabilityValue,
                setValue:    setRepeatabilityValue,
                factor:      repeatabilityFactor,
                setFactor:   setRepeatabilityFactor,
              },
            ].map(row => (
              <div key={row.label} className="flex items-center gap-4">
                ...
                <input
                  ...
                  className="border border-border rounded-md px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-brand-orange focus:border-brand-orange transition-colors w-56 bg-white"
                  placeholder=""
                />
                <label className="text-xs font-medium text-text-secondary shrink-0 ml-6">
                  Factor Reqirment:
                </label>
                <select
                  value={row.factor}
                  onChange={e => row.setFactor(e.target.value)}
                  className="border border-border rounded-md px-3 py-2 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-brand-orange focus:border-brand-orange transition-colors w-32 bg-white appearance-none cursor-pointer"
                >
                  <option value="">--</option>
                  {FACTOR_OPTIONS.map(f => (
                    <option key={f} value={f}>{f}</option>
      ...
      <div className="bg-white rounded-xl border border-border shadow-sm">
        <div className="px-5 py-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-text-primary">Data Table</h2>
